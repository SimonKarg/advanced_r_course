...
Define a function that calculates standard error for a vector of numbers.

Bonus: make it so you can specify na.rm = TRUE

Bonus: make it so that it gives some informative errors when given wrong input (e.g. a character value). Tip: check out the `stopifnot()` function, or use an if statement 
